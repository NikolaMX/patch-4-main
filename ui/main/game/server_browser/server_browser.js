...
$(document).ready(function () {
    ...
    function ServerBrowserViewModel() {
        ...
        self.allGames = ko.computed(function () {
            lanAndCustomGamesUUID = new Set();
            lanAndCustomGames = new Array();
            
            // We could use a Map here and just convert the .values() to array
            // but Array.from is not available.
            _.forEach(self.lanGameList().concat(self.customGameList()), function (game) {
                if (!lanAndCustomGamesUUID.has(game.uuid)) {
                    lanAndCustomGamesUUID.add(game.uuid);
                    lanAndCustomGames.push(game);
                }
            });

            return self.gameList().concat(lanAndCustomGames);
        });
        ...
        self.processGameBeacon = function (beacon, region, lobby_id, host, port) {
            try { /* server modding allows users to modify the structure of the beacon,
                ...
                var titans = _.contains(requiredContent, 'PAExpansion1');

                if (titans)
                {
                    if (!self.usingTitans)
                        icons.push( {title: 'Titans Expansion', img: 'coui://ui/main/shared/img/titans-icon.png'});
                }
                else if (self.usingTitans)
                     icons.push({ title: 'Classic PA', img: 'coui://ui/main/shared/img/classic-icon.png'});
                ...
                if (count > 0)
                {
                    for (var i = 0; i < count; i++)
                    {
                        ...
                        var icon = self.modIcons[identifier];
    ...
    $('#getPassword').modal().on('shown.bs.modal', function()
    {
